...
contract HyFiHookSetProtocolFeeTest is HyFiHookSharedSetup {
    ...
    function test_setProtocolFee_setsValue() public {
        hook.setProtocolFee(1000); // 0.1%
        assertEq(hook.protocolFeePips(), 1000);
    }

    function test_setProtocolFee_zero() public {
        hook.setProtocolFee(1000);
        hook.setProtocolFee(0);
        assertEq(hook.protocolFeePips(), 0);
    }

    function test_setProtocolFee_maxValue() public {
        hook.setProtocolFee(1_000_000); // 100% = FEE_DENOM
        assertEq(hook.protocolFeePips(), 1_000_000);
    }

    // ─── Revert: fee too high ────────────────────────────────────────────

    function test_setProtocolFee_RevertWhen_exceedsDenom() public {
        vm.expectRevert(HyFiHook.FeeTooHigh.selector);
        hook.setProtocolFee(1_000_001);
    }
    ...
    function test_setProtocolFee_feeDeductedFromMMInputOnSwap_oneForZero() public {
