...
contract HyFiHookWithdrawFrom6909Test is HyFiHookSharedSetup {
    ...
    function test_withdrawFrom6909_native_decreasesClaims() public {
        ...
        vm.prank(mm1);
        hook.withdrawFrom6909(native, amount);
    ...
    function test_withdrawFrom6909_native_entireBalance() public {
        ...
        vm.prank(mm1);
        hook.withdrawFrom6909(native, fullBal);
    ...
    function test_withdrawFrom6909_ERC20() public {
        uint256 amount = 10e6;
        ...
        vm.prank(mm1);
        hook.withdrawFrom6909(usdc, amount);
    ...
    function test_withdrawFrom6909_RevertWhen_insufficientBalance() public {
        uint256 balance = hook.lpBalances(mm1, native);

        vm.prank(mm1);
        vm.expectRevert(HyFiHook.InsufficientBalance.selector);
        hook.withdrawFrom6909(native, balance + 1);
    }

    function test_withdrawFrom6909_RevertWhen_zeroBalance() public {
        address mm2 = makeAddr("mm2");
        hook.addToWhitelist(mm2);

        vm.prank(mm2);
        vm.expectRevert(HyFiHook.InsufficientBalance.selector);
        hook.withdrawFrom6909(native, 1);
    }

    // ─── Isolation: withdraw does not touch another MM's balance ─────────

    function test_withdrawFrom6909_doesNotAffectOtherMM() public {
        address mm2 = makeAddr("mm2");
        hook.addToWhitelist(mm2);
        vm.deal(mm2, 100e18);
        vm.prank(mm2);
        ...
        vm.prank(mm1);
        hook.withdrawFrom6909(native, 5e18);
    ...
    function test_withdrawFrom6909_afterSwapReducesBalance() public {
        ...
        vm.prank(mm1);
        hook.withdrawFrom6909(usdc, usdcBalAfterSwap);
