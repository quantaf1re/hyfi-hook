...
contract HyFiHookRescueTokenTest is HyFiHookSharedSetup {
    ...
    function test_rescueToken_transfersERC20ToOwner() public {
        uint256 amount = 100e6;
        deal(USDC_ADDR, address(hook), amount);

        uint256 ownerBefore = usdc.balanceOf(owner);
        hook.rescueToken(usdc, amount);
    ...
    function test_rescueToken_partialAmount() public {
        uint256 total = 200e6;
        uint256 withdrawAmt = 50e6;
        deal(USDC_ADDR, address(hook), total);

        uint256 ownerBefore = usdc.balanceOf(owner);
        hook.rescueToken(usdc, withdrawAmt);
    ...
    function test_rescueToken_native_transfersToOwner() public {
        uint256 amount = 5e18;
        vm.deal(address(hook), amount);

        uint256 ownerBefore = owner.balance;
        hook.rescueToken(native, amount);
    ...
    function test_rescueToken_native_partialAmount() public {
        uint256 total = 10e18;
        uint256 withdrawAmt = 3e18;
        vm.deal(address(hook), total);

        uint256 ownerBefore = owner.balance;
        hook.rescueToken(native, withdrawAmt);
    ...
    function test_rescueToken_RevertWhen_notOwner() public {
        ...
        hook.rescueToken(usdc, 100e6);
    }

    function test_rescueToken_native_RevertWhen_notOwner() public {
        vm.deal(address(hook), 5e18);

        address notOwner = address(0xdead);
        vm.prank(notOwner);
        vm.expectRevert(abi.encodeWithSelector(OwnableUpgradeable.OwnableUnauthorizedAccount.selector, notOwner));
        hook.rescueToken(native, 5e18);
    }
}
